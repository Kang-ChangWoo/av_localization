...
def main() -> int:
    ...
    #
    # Three scalars. The rule once had four, with a second gate on the acoustic
    # margin and a relative-evidence transform of the acoustic summary. Both
    # were removed after a room-held-out ablation showed the stripped rule
    # matching or beating the full one on every backbone (feasible/results/
    # H_simple_rule.md against H_fixed_structure.md); the full rule survives
    # as the `full` row of the fusion ablation below. tau_a at -2.0 is the
    # acoustic gate held open, since the margin is never negative.
    scalars = list(itertools.product((0.5, 1.0, 2.0),                    # weight
                                     (0.02, 0.05, 0.1),                  # sigmoid
                                     (0.005, 0.02, 0.05, 0.1, 0.2)))     # tau_v
    SIMPLE = dict(tau_a=-2.0, ac_transform="standard")
    FULL_TAU_A = (-2.0, 0.0, 0.2, 0.4)

    def tune(tag, ve, ae, cond=None):
        b, bs = None, -1.0
        for w, s, tv in scalars:
            cfg = ModeFusionConfig(vis_evidence=ve, ac_evidence=ae, rule="continuous",
                                   weight=w, sigmoid_scale=s, tau_v=tv, **SIMPLE)
    ...
    MD(f"\nShared by every backbone: the formula, the visual-ambiguity gate, visual "
       f"evidence `{struct[0]}` and acoustic evidence `{struct[1]}`. Three scalars are "
    ...
    shared = dict(vis_evidence=pu.vis_evidence, ac_evidence=pu.ac_evidence,
                  ...
                  tau_v=pu.tau_v, tau_a=pu.tau_a, ac_transform=pu.ac_transform)
    # the full rule is refit on the fit collection over its own, larger grid,
    # so the ablation compares two tuned rules rather than one tuned and one
    # handed the other's scalars
    full, fs = None, -1.0
    for w, s, tv in scalars:
        for ta in FULL_TAU_A:
            c = MFC(vis_evidence=pu.vis_evidence, ac_evidence=pu.ac_evidence,
                    rule="continuous", weight=w, sigmoid_scale=s, tau_v=tv,
                    tau_a=ta, ac_transform="relative")
            sc = float((evaluate("unlocSTFT", args.condition, c, True)[0] < 1).mean())
            if sc > fs:
                full, fs = c, sc
    for rule, nm, cfg, bold in (
            ("mode_rerank", "rerank, unconditional", MFC(**dict(shared, rule="mode_rerank")), False),
            ("relative", "weighted sum, no gate", MFC(**dict(shared, rule="relative")), False),
            ("selective", "hard gate on visual ambiguity", MFC(**dict(shared, rule="selective")), False),
            ("continuous", "+ acoustic gate, relative evidence (4 scalars)", full, False),
            ("continuous", "smooth gate on visual ambiguity (ours, 3 scalars)",
             MFC(**dict(shared, rule="continuous")), True)):
        e, o, _, _ = evaluate("unlocSTFT", args.condition, cfg, False)
